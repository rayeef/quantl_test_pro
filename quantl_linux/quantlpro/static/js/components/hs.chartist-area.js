					...
					if (isOnceCreatedTrue == 1) {
						...
						$(el).find('.ct-series').each(function (i2) {
							if (optFillColors && !optIsFillColorsGradient) {
								...
								areaChartStyles += '#areaCharts' + i + ' .ct-series:nth-child(' + (i2 + 1) + ') .ct-point {border-color: ' + optFillColors[i2][0] + '}'
							}

							if (optPointsColors) {
								areaChartStyles += '#areaCharts' + i + ' .ct-series:nth-child(' + (i2 + 1) + ') .ct-point {border-color: ' + optPointsColors[i2] + '}'
							}
